...
const debugLog = e => process.env.OAUTH_DEBUG === 'enabled' && console.log(`NUXT-OAUTH: ${e}`)

Handler.prototype.createAuth = function createAuth () {
  debugLog('In createAuth')
  const {
    oauthHost,
    ...
    oauthRedirectHost: redirectHost,
    oauthRedirectProtocol: redirectProtocol,
    scopes = []
  } = this.opts
  const redirect = redirectHost || this.req.headers.host
  const protocol = redirectProtocol || this.req.headers['x-forwarded-proto'] || this.req.headers['X-Forwarded-Proto'] || 'http'
  ...
  return new OAuth2({
    ...
    clientId,
    clientSecret,
    redirectUri: `${protocol}://${redirect}/auth/callback`,
    scopes
  })
}
...
Handler.prototype.createSession = function createSession () {
  debugLog('In createSession')
  if (this.req[this.opts.sessionName]) return Promise.resolve()
  const cookieDomain = (this.opts.oauthRedirectHost || this.req.headers.host).replace(/:\d*/, '')
  debugLog(`createSession - cookie domain: ${cookieDomain}`)
...
Handler.prototype.checkRequestAuthorization = async function checkRequestAuthorization () {
  ...
  debugLog(`In checkRequestAuthorization. Existing token: ${existingToken}`)
  try {
    ...
    debugLog(`checkRequestAuthorization Error: ${e}`)
...
Handler.prototype.authenticateCallbackToken = async function authenticateCallbackToken () {
  debugLog(`In authenticateCallbackToken - url: ${this.req.url}`)
  let redirectUrl
  try {
    const { state } = parse(this.req.url.split('?')[1])
    redirectUrl = JSON.parse(atob(state)).redirectUrl
  } catch (e) {
    debugLog(`authenticateCallbackToken (1st block) Error: ${e}`)
    errorLog(e)
    redirectUrl = '/'
  }
  debugLog(`authenticateCallbackToken: redirectUrl determined: ${redirectUrl}`)
  try {
    debugLog(`authenticateCallbackToken - about to retrieve token from URL: ${this.req.url}`)
    const token = await this.auth.code.getToken(this.req.url)
    debugLog(`authenticateCallbackToken: Token retrieved from url`)
    const { accessToken, refreshToken, expires } = token
    debugLog(`accessToken ${accessToken}, refreshToken ${refreshToken}, expires ${expires}`)
    await this.saveData({ accessToken, refreshToken, expires })
    return this.redirect(redirectUrl)
  } catch (e) {
    errorLog(e)
    debugLog(`authenticateCallbackToken (2nd block) Error: ${e}`)
    return this.redirectToOAuth(redirectUrl)
  }
}

Handler.prototype.saveData = async function saveData (token) {
  debugLog('In saveData')
  ...
  debugLog(`saveData - accessToken: ${JSON.stringify(accessToken)}`)

  const fetchUser = async () => {
    try {
      return await this.opts.fetchUser(accessToken, this.req, this.opts)
    } catch (e) {
      debugLog(`saveData (fetchUser) Error: ${e}`)
      return {}
    }
  }
  ...
  this.req.user = user
  debugLog(`saveData - retrieved user: ${JSON.stringify(user)}`)
  return true
}
...
Handler.prototype.authenticate = async function authenticate () {
  debugLog('In authenticate')
  await this.createSession()

  const { accessToken, refreshToken } = this.getSessionToken()

  if (!accessToken) {
    debugLog('authenticate - no access token')
    return null
  }
  ...
  try {
    ...
    debugLog(`authenticate - token: ${JSON.stringify(token)}`)
...
Handler.prototype.useRefreshToken = async function useRefreshToken () {
  ...
  try {
    ...
    debugLog(`useRefreshToken Error: ${e}`)
    errorLog(e)
    return null
  }
}
